from loss_function.adv import *


class DAANLoss(AdversarialLoss, LambdaSheduler):
    def __init__(self, num_class, gamma=1.0, max_iter=1500, init_dynamic_factor=1.0, **kwargs):
        ...
        self.dynamic_factor = init_dynamic_factor
    ...
    def forward(self, source, target, source_logits, target_logits):
        ...
        source_ratio, target_ratio = len(source) / (len(source) + len(target)), len(target) / (len(source) + len(target))
        global_loss = source_ratio * source_loss_g + target_ratio * target_loss_g
        local_loss = 0.5 * (source_loss_l + target_loss_l)
    ...
    def update_dynamic_factor(self, epoch_length):
        if self.d_g == 0 and self.d_l == 0:
            ...
            # print(self.d_g, self.d_l, self.dynamic_factor)
        self.d_g, self.d_l = 0, 0
